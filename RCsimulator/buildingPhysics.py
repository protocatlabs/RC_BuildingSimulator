...
#        self.coolingEfficiency=coolingEfficiency
    ...
    def calc_heatflow(self,theta_e, phi_int, phi_sol, phi_hc_nd):
...
#        self.phi_st=(1-(self.A_m/self.A_t)-(self.h_tr_w/(9.1*self.A_t)))*(0.5*phi_int+phi_sol)
    ...
    def calc_temperatures_crank_nicolson(self, phi_hc_nd, phi_int, phi_sol, theta_e, theta_m_prev):
        ...
        self.calc_heatflow(theta_e, phi_int, phi_sol, phi_hc_nd)

        self.calc_phi_m_tot(theta_e)

        self.calc_theta_m_t(theta_m_prev)  #calculates the new bulk temperature POINT from the old one

        self.calc_theta_m(theta_m_prev)  #calculates the AVERAGE bulk temperature used for the remaining calculation
        ...
        return self.theta_m, self.theta_air, self.theta_op
    ...
    def solve_building_energy(self,phi_int, phi_sol,theta_e, theta_m_prev):
        ...
        if not self.has_heating_demand and not self.has_cooling_demand:
            ...
            if self.has_heating_demand:
                supDirector.setBuilder(self.heatingSupplySystem(Load=self.phi_hc_nd_ac, theta_e=theta_e, heatingSupplyTemperature=self.heatingSupplyTemperature, coolingSupplyTemperature=self.coolingSupplyTemperature, has_heating_demand=self.has_heating_demand, has_cooling_demand=self.has_cooling_demand))  
